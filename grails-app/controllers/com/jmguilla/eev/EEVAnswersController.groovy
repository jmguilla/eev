...
class EEVAnswersController {
  ...
  def sendPDF(){
    withFormat{
      json{
        ...
        if(!eev){
          response.status = 404
          respond([type: 'danger', content: "Aucun EEV ne correspond ‡ l'id ${params.id}"])
        }
        try{
          sendMail {
            to springSecurityService.getCurrentUser().email
            subject "EEV #${params.id} du ${eev.creationDate} pour '${eev.interviewee?:'Inconnu'}'"
            text "Consultable ici: ${g.createLink(controller: 'EEVAnswers', action: 'show', id: params.id)}.pdf Et prÈsent en piËce jointe."
          }
          respond([type: 'success', content: 'Email envoyÈ'])
        }catch(Throwable t){
          response.status = 500
      ...
      json{
        try{
          ...
          if(!request.JSON.answers){
            respond([type: "danger", content: "Vous n'avez r√©pondu √† aucune question..."])
            return
          }
  ...
  def deleteEEV(){
    if(!request.method.equalsIgnoreCase('DELETE')){
      response.status = 400
      respond([type: 'danger', content: 'Seulement la m√©thode DELETE est authoris√©e.'])
    }
    if(!params.id){
      response.status = 400
      respond([type: 'danger', content: 'Un id doit √™tre fourni pour la suppression.'])
    }
    ...
    eev.delete()
    respond([type: 'success', content: "L\'EEV #${params.id} a √©t√© supprim√©."])
  }
