	...
	<body>
		...
		<div class="col-xs-12" ng-app="eev" ng-controller="EEVAnswersListCtrl">
			...
			<ol ng-cloak>
				...
				<a ng-if="!!eev.interviewee == true" href="${createLink(controller: 'EEVAnswers', action: 'show')}/{{eev.id}}">{{eev.interviewee}} - {{eev.creationDate}}</a>
				<a ng-if="!!eev.interviewee == false" href="${createLink(controller: 'EEVAnswers', action: 'show')}/{{eev.id}}">Vendeur inconnu - {{eev.creationDate}}</a>
				&nbsp;<a href="${createLink(controller: 'EEVAnswers', action: 'toPdf')}/{{eev.id}}" target="_blank">pdf</a>
				</li>
