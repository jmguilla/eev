	...
	<body>
		...
		<div class="col-xs-12" ng-app="eev" ng-controller="EEVAnswersListCtrl">
			...
			<tr>
				<td>#</td>
				<td>PDF</td>
				<td><a ng-click="predicate = 'synced'; reverse=!reverse">Sync</a></td>
				<td><a ng-click="predicate = 'interviewee'; reverse=!reverse">Vendeur</a></td>
				<td><a ng-click="predicate = 'creationDate'; reverse=!reverse">Date</a></td>
				<td></td>
			</tr>
			<tr ng-repeat="eev in eevs|orderBy:predicate:reverse" ng-cloak>
				...
				<td ng-if="eev.synced == true"><span style="color: #3c763d;" class="glyphicon glyphicon-envelope" ng-show="!!pdf_sent[$index] == true"></span><a ng-hide="!!pdf_sent[$index] == true" ng-click="sendPDF(eev.id, $index)"><img alt="pdf" src="${resource(dir: 'images', file: 'pdfdownload.png')}" width="15px" height="auto"/></a></td>
				...
				<td><a ng-click="deleteEEV(eev)"><span class="glyphicon glyphicon-remove" class="glyphicon glyphicon-user" style="color: #a94442;"></a></span></td>
			</tr>
			</table>
		</div>
	</body>
